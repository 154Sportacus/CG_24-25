...
std::vector<Vertex> generateSphere(float radius, int slices, int stacks) {
    ...
    // Note: Removed the shift that moved the sphere upward.
    // This way, the sphere is centered at the origin with y in [-radius, radius].
    return verts;
}
...
std::vector<Vertex> generateCube(float dimension, int divisions) {
    ...
    // Note: Removed the upward shift (adding half) so that the cube is centered at the origin.
    return verts;
}
