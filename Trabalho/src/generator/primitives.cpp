...
std::vector<Vertex> generateSphere(float radius, int slices, int stacks) {
    ...
    for (int i = 0; i < stacks; i++) {
        ...
        for (int j = 0; j < slices; j++) {
            ...
            // First triangle.
            float x1 = radius * cos(lat1) * cos(angle);
            float y1 = radius * sin(lat1);
            float z1 = radius * cos(lat1) * sin(angle);
            float x2 = radius * cos(lat2) * cos(angle);
            float y2 = radius * sin(lat2);
            float z2 = radius * cos(lat2) * sin(angle);
            float x3 = radius * cos(lat2) * cos(angle2);
            float y3 = radius * sin(lat2);
            float z3 = radius * cos(lat2) * sin(angle2);
            verts.push_back({x1, y1, z1});
            verts.push_back({x2, y2, z2});
            verts.push_back({x3, y3, z3});
            // Second triangle.
            float x6 = radius * cos(lat1) * cos(angle2);
            float y6 = radius * sin(lat1);
            float z6 = radius * cos(lat1) * sin(angle2);
            verts.push_back({x1, y1, z1});
            verts.push_back({x3, y3, z3});
...
std::vector<Vertex> generateCone(float bottomRadius, float height, int slices, int stacks) {
    ...
    for (int j = 0; j < slices; j++){
        ...
        verts.push_back({bottomRadius * cos(angle1), 0.0f, bottomRadius * sin(angle1)});
        verts.push_back({0.0f, 0.0f, 0.0f});
        verts.push_back({bottomRadius * cos(angle0), 0.0f, bottomRadius * sin(angle0)});
    }
    // Lateral surface.
    for (int i = 0; i < stacks; i++){
        ...
        for (int j = 0; j < slices; j++){
            float angle0 = 2 * M_PI * j / slices;
            float angle1 = 2 * M_PI * (j + 1) / slices;
            float x0 = r0 * cos(angle0), z0 = r0 * sin(angle0);
            float x1 = r0 * cos(angle1), z1 = r0 * sin(angle1);
            float x2 = r1 * cos(angle1), z2 = r1 * sin(angle1);
            float x3 = r1 * cos(angle0), z3 = r1 * sin(angle0);
            if (i < stacks - 1) {
